...
### C. Graph Neural Networks (Structural Learning)
**Files**: `graph_converter.py`, `gnn.py`
We go beyond simple node counts by representing code as a **Graph** and learning structural embeddings using GNNs.
-   **Graph Construction**: The AST is converted into a graph where:
    -   **Nodes**: AST elements (e.g., `If`, `For`, `Call`).
    -   **Edges**: Parent-child relationships (control flow).
    -   **Features**: One-hot encoding of the node type.
-   **Model**: A 3-layer **Graph Convolutional Network (GCN)** implemented with `torch_geometric`.
-   **Training (Self-Supervised)**:
    -   We train the GNN to **predict the risk score** (generated by `rules.py`).
    -   This forces the model to learn *which* structures correspond to risky code patterns.
    -   We extract the 32-dimensional internal vector as the "Structural Embedding".
